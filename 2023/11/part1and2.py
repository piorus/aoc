...
def measure(expansion_multiplier):
    ...
    for i, p1 in enumerate(galaxies):
        for p2 in galaxies[i + 1:]:
            ...
            solution += (
                    (a + b - x_expansion - y_expansion)
                    + (x_expansion * expansion_multiplier)
                    + (y_expansion * expansion_multiplier)
            )

    return solution


print('part 1 = {}'.format(measure(expansion_multiplier=2)))
print('part 2 = {}'.format(measure(expansion_multiplier=1000000)))
